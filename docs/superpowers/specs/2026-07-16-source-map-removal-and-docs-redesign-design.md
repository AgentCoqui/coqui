...
5. ~~Cross-repo risk: docs-site frontmatter.~~ **RESOLVED 2026-07-16 — not a blocker.** The docs site (`docs.agentcoqui.com`, repo `docs-coquibot-org`) syncs `Core/coqui/docs/*.md` into a Nextra `content/**` tree via `scripts/sync-docs.mjs`, whose header states: *"Strip source frontmatter and generate Nextra frontmatter locally"* (`:16`). `stripFrontmatter()` (`:365`) is already called on every synced doc (`:472`) — defensively, before any doc has frontmatter. **Adding frontmatter to `docs/*.md` is a no-op for the published site.** Proceed with frontmatter as specified.

   Two facts from that investigation belong in the plan:
   - **The website's `DOC_ROUTES` table is a third hand-maintained list** of the same per-doc metadata (`{sourcePath, dest, title, description, section, render}`, `:70+`), duplicating the generator allowlist this spec deletes. It maps **21 docs and is missing only `QUESTIONS.md`**, where `documentation.json` is missing eight — the public, reviewed list stayed accurate while the internal, unreviewed one rotted. Once core docs carry frontmatter, `DOC_ROUTES` should read `title`/`description` from source rather than restating them. That is **out of scope here** (separate repo) and is being handed to the docs agent.
   - `validateSourceDocs()` (`:742`, called `:787`) **throws when a mapped source doc is missing**. This change deletes no file under `docs/`, so no route breaks — but any future doc deletion must drop its `DOC_ROUTES` entry in the same beat or the docs-site build breaks (the `CHANNELS.md` precedent).
