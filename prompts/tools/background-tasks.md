...
1. **Write detailed prompts.** The background task agent has no access to the current conversation. Include all relevant file paths, requirements, and constraints directly in the prompt.
2. **Set a descriptive title.** Titles appear in task listings and help users identify what each task does.
3. **Check status periodically.** Use `task_status` to report progress when the user asks.
