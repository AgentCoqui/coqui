...
/**
 * Observes agent events and persists them to the task_events table.
 *
 * Used by background task processes to create a durable event log
 * that the API server can poll for real-time status updates.
 */
final class BackgroundTaskObserver implements SplObserver
{
    ...
    private function handleEvent(string $event, mixed $data): void
    {
        ...
        // Map internal event names to SSE-compatible event types
        ...
        try {
            $this->storage->appendTaskEvent($this->taskId, $eventType, $eventData);
        } catch (\Throwable) {
