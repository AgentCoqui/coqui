...
final class BackgroundTaskManager
{
     ...
     * the boot window so a slow-booting task is not mistaken for a crash.
     */
     ...
     * which detaches the real `task:run` child into its own session and then exits
     ...
     * whether the task actually finished. We consult the DB status and the child's
     ...
     * status.
     *
     * Uses conditional UPDATE (WHERE status = 'running') so we never overwrite a
     * status the child already committed.
     */
    private function reapFinishedProcesses(): void
    {
        foreach ($this->processes as $taskId => $process) {
            ...
            $task = $this->storage->getTask($taskId);
            $dbStatus = is_array($task) ? (string) ($task['status'] ?? '') : '';

            // The child records its own terminal status (TaskRunCommand). If it
            // already did, the task is genuinely done — just release the handle.
            if ($dbStatus === 'completed' || $dbStatus === 'failed' || $dbStatus === 'cancelled') {
                $this->releaseProcess($taskId);
                continue;
            }
            ...
            // captured at spawn.
            $wrapperPid = $this->pids[$taskId] ?? 0;
            $childPid = is_array($task) ? (int) ($task['pid'] ?? 0) : 0;
            $childPidRecorded = $childPid > 0 && $childPid !== $wrapperPid;

            if ($childPidRecorded && ProcessSpawner::isProcessAlive($childPid)) {
                // Real task:run child is still executing — check again next tick.
                continue;
            }
            ...
            if (!$childPidRecorded) {
                ...
                $firstSeen = $this->wrapperExitedAt[$taskId] ??= microtime(true);
    ...
    /**
     * Release all tracking for a task process: close the handle/pipes and clear
     * the wrapper-exit timestamp.
     */
    private function releaseProcess(string $taskId): void
    {
        $this->closeProcess($taskId);
        unset($this->wrapperExitedAt[$taskId]);
    }
